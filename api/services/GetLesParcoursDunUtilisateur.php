...
if ($lang == "xml") {
    ...
    $donnees = creerFluxXML($msg, $lesTracesUtilisateur);
}
else {
    $content_type = "application/json; charset=utf-8";      // indique le format Json pour la réponse
    $donnees = creerFluxJSON($msg, $lesTracesUtilisateur);
}
...
function creerFluxXML($msg, $lesTracesUtilisateur)
{
    /* Exemple de code XML
     <?xml version="1.0" encoding="UTF-8"?>
     <!--Service web GetTouslesTracesUtilisateur - BTS SIO - Lycée De La Salle - Rennes-->
     <data>
     <reponse>2 utilisateur(s).</reponse>
     <donnees>
     <lesTracesUtilisateur>
     <utilisateur>
     <id>2</id>
...
function creerFluxJSON($msg, $lesTracesUtilisateur)
{
